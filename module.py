...
def global_noise(z):
    ...
    z=tf.tile(z,[1,256,256,1])  # [1,256,256,3] ,batchsize=1, noise_dim=3
    return z


def periodic_noise(k,phi,dp):
    a=[i for i in range(256)]
    a=tf.expand_dims(a,1)
    a=tf.expand_dims(a,2)
    a=tf.tile(a,[1,256,dp])

    b=[i for i in range(256)]
    b=tf.expand_dims(b,0)
    b=tf.expand_dims(b,2)
    b=tf.tile(b,[256,1,dp])

    c=tf.stack((a,b),axis=0)

    k=tf.expand_dims(k,1)
    k=tf.expand_dims(k,2)
    k=tf.tile(k,[1,256,256,1])
    c=tf.cast(c,tf.float32)
    z=tf.sin(tf.reduce_sum(c*k,0)+phi)
    z=tf.expand_dims(z,0) # add first batch_size dim
    return z
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        c4 = tf.nn.relu(batch_norm(conv2d(c3, options.gf_dim*8, 3, 2, name='g_e4_c'), 'g_e4_bn'))
        c5 = tf.nn.relu(batch_norm(conv2d(c4, options.gf_dim*8, 3, 2, name='g_e5_c'), 'g_e5_bn'))
        # define G network with 9 resnet blocks
        r1 = residule_block(c5, options.gf_dim*8, name='g_r1')
        r2 = residule_block(r1, options.gf_dim*8, name='g_r2')
        r3 = residule_block(r2, options.gf_dim*8, name='g_r3')
        r4 = residule_block(r3, options.gf_dim*8, name='g_r4')
        r5 = residule_block(r4, options.gf_dim*8, name='g_r5')
        r6 = residule_block(r5, options.gf_dim*8, name='g_r6')
        r7 = residule_block(r6, options.gf_dim*8, name='g_r7')
        r8 = residule_block(r7, options.gf_dim*8, name='g_r8')
        r9 = residule_block(r8, options.gf_dim*8, name='g_r9')
        ...
        d3 = tf.nn.relu(batch_norm(d3, 'g_d3_bn'))
        d4 = deconv2d(d3, options.gf_dim, 3, 2, name='g_d4_dc')
        d4 = tf.nn.relu(batch_norm(d4, 'g_d4_bn'))
...
def generator_resnet_origin(image, options, reuse=False, name="generator"):
